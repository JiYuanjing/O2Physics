...
AnalysisCompositeCut* o2::aod::dqcuts::GetCompositeCut(const char* cutName)
{
  ...
  if (!nameStr.compare("protonPIDPV10")) {
    cut->AddCut(GetAnalysisCut("protonPID_TPCnTOF7"));
    cut->AddCut(GetAnalysisCut("protonPVcut2"));
    return cut;
  }

  if (!nameStr.compare("protonPIDPV_TPC")) {
    // cut->AddCut(GetAnalysisCut("protonPID_TPCnTOF7"));
    cut->AddCut(GetAnalysisCut("protonPVcut2"));
    return cut;
  }

  if (!nameStr.compare("protonPIDPV_TOF")) {
    cut->AddCut(GetAnalysisCut("protonPID_TPCnTOF5"));
    cut->AddCut(GetAnalysisCut("protonPVcut2"));
    return cut;
  }
...
AnalysisCut* o2::aod::dqcuts::GetAnalysisCut(const char* cutName)
{
  ...
  if (!nameStr.compare("LooseGlobalTrackRun3_PbPb2")) {
    cut->AddCut(VarManager::kIsSPDany, 0.5, 1.5);
    cut->AddCut(VarManager::kTPCchi2, 0.0, 4.0);
  ...
  if (!nameStr.compare("LooseGlobalTrackRun3_PbPb3")) {
    cut->AddCut(VarManager::kIsSPDany, 0.5, 1.5);
    cut->AddCut(VarManager::kTPCchi2, 0.0, 4.0);
    cut->AddCut(VarManager::kTPCncls, 70., 161.);
    cut->AddCut(VarManager::kTrackDCAxy, -0.5, 0.5);
  ...
  if (!nameStr.compare("protonPVcut_tight")) {
    cut->AddCut(VarManager::kTrackDCAxy, -0.005, 0.005);
    cut->AddCut(VarManager::kTrackDCAz, -0.005, 0.005);
    // cut->AddCut(VarManager::kPt, 0.4, 3);
    cut->AddCut(VarManager::kPin, 0.25, 3);
    cut->AddCut(VarManager::kEta, -0.9, 0.9);
    cut->AddCut(VarManager::kIsSPDany, 0.5, 1.5);
  ...
  if (!nameStr.compare("protonPVcut2")) {
    cut->AddCut(VarManager::kTrackDCAxy, -0.1, 0.1);
    cut->AddCut(VarManager::kTrackDCAz, -0.15, 0.15);
    cut->AddCut(VarManager::kPt, 0.4, 3);
    cut->AddCut(VarManager::kEta, -0.9, 0.9);
  ...
  if (!nameStr.compare("pionPID_TPCnTOF_pure")) {
    cut->AddCut(VarManager::kTPCnSigmaPi, -2.0, 2.0);
    cut->AddCut(VarManager::kTOFnSigmaPi, -2.0, 2.0);
    cut->AddCut(VarManager::kP, 0.0, 2.0);
    return cut;
  }
  ...
  if (!nameStr.compare("protonPID_TPCnTOF3")) {
    cut->AddCut(VarManager::kTPCnSigmaPr, -2.5, 2.5);
    cut->AddCut(VarManager::kTOFnSigmaPr, -2.5, 2.5, false, VarManager::kPin, 0.9, 1e+10, false);
    return cut;
  }
  if (!nameStr.compare("protonPID_TPCnTOF3_tight")) {
    cut->AddCut(VarManager::kTPCnSigmaPr, -2., 2.);
    cut->AddCut(VarManager::kTOFnSigmaPr, -2., 2., false, VarManager::kPin, 0.8, 1e+10, false);
    return cut;
  }

  if (!nameStr.compare("protonPID_TPCnTOF3_2")) {
    cut->AddCut(VarManager::kTPCnSigmaPr, -2.5, 2.5, false, VarManager::kPin, 0.0, 0.9, false);
    cut->AddCut(VarManager::kTPCTOFnSigmaPr, 0.0, 2.5, false, VarManager::kPin, 0.9, 1e+10, false);
    return cut;
  }

  if (!nameStr.compare("protonPID_TPCnTOF4")) {
    cut->AddCut(VarManager::kTPCnSigmaPr, -2.5, 2.5);
    cut->AddCut(VarManager::kTOFnSigmaPr, -2.5, 2.5);
    return cut;
  }
  ...
  if (!nameStr.compare("protonPID_TPCnTOF7")) {
    ...
    cut->AddCut(VarManager::kTOFnSigmaPr, -2, 4, false, VarManager::kPt, 0.8, 1e+10, false);
    return cut;
  }

...
struct AnalysisSameEventPairing {
  ...
  void init(o2::framework::InitContext& context)
  {
    ...
    fEnableBarrelMixingHistos = context.mOptions.get<bool>("processMixingAllSkimmed") || context.mOptions.get<bool>("processMixingBarrelSkimmed") || context.mOptions.get<bool>("processMixingBarrelSkimmedFlow") || context.mOptions.get<bool>("processMixingBarrelWithQvectorCentrSkimmedNoCov") || context.mOptions.get<bool>("processMixingBarrelSkimmedFemto");
  ...
  // barrel-barrel and muon-muon event mixing
  template <int TPairType, uint32_t TEventFillMap, typename TEvents, typename TAssocs, typename TTracks>
  void runSameSideMixingFemto(TEvents& events, TAssocs const& assocs, TTracks const& tracks, Preslice<TAssocs>& preSlice)
  {
    events.bindExternalIndices(&assocs);
    int mixingDepth = fConfigMixingDepth.value;
    fAmbiguousPairs.clear();
  ...
  // Yuanjing add for femto
  PROCESS_SWITCH(AnalysisSameEventPairing, processMixingBarrelSkimmedFemto, "Run barrel type mixing pairing, with skimmed tracks, with output trees", false);
...
struct AnalysisDileptonTrack {
  ...
  Configurable<float> fConfigDileptonLxyCutHigh{"cfgDileptonLxyCutHigh", 0.0, "Lxy cut for dileptons used in the triplet vertexing"};
  Configurable<float> fConfigDileptonLxyCutLow{"cfgDileptonLxyCutLow", 0.0, "Lxy cut for dileptons used in the triplet vertexing"};
  Configurable<bool> fConfigUseKFVertexing{"cfgUseKFVertexing", false, "Use KF Particle for secondary vertex reconstruction (DCAFitter is used by default)"};
  ...
  Filter dileptonFilter = aod::reducedpair::pt > fConfigDileptonLowpTCut&& aod::reducedpair::pt<fConfigDileptonHighpTCut && aod::reducedpair::mass> fConfigDileptonLowMass&& aod::reducedpair::mass<fConfigDileptonHighMass && nabs(aod::reducedpair::sign) == fConfigDileptonSign && aod::reducedpair::lxy> fConfigDileptonLxyCut && aod::reducedpair::lxy> fConfigDileptonLxyCutLow && aod::reducedpair::lxy<fConfigDileptonLxyCutHigh;
  ...
  void init(o2::framework::InitContext& context)
  {
    ...
    if (context.mOptions.get<bool>("processDummy")) {
      ...
      if (isBarrel || isBarrelFemto || isBarrelME || isBarrelJpsiProtonMEFemto || isBarrelAsymmetric || isMuon || isMuonME) {
  ...
  // add by Yuanjing
  // Template function to run pair - hadron combinations, femto
  template <int TCandidateType, uint32_t TEventFillMap, uint32_t TTrackFillMap, typename TEvent, typename TTracks, typename TTrackAssocs, typename TDileptons>
  void runDileptonHadronFemto(TEvent const& event, TTrackAssocs const& assocs, TTracks const& tracks, TDileptons const& dileptons)
  {
    VarManager::ResetValues(0, VarManager::kNVars, fValuesHadron);
    VarManager::ResetValues(0, VarManager::kNVars, fValuesDilepton);
    VarManager::FillEvent<TEventFillMap>(event, fValuesHadron);
                            ...
                            MyBarrelTracksWithCov const& tracks, soa::Filtered<MyDielectronCandidates> const& dileptons)
  {
    ...
    for (auto& event : events) {
      auto groupedBarrelAssocs = assocs.sliceBy(trackAssocsPerCollision, event.globalIndex());
      auto groupedDielectrons = dileptons.sliceBy(dielectronsPerCollision, event.globalIndex());
      runDileptonHadronFemto<VarManager::kJpsiEEProton, gkEventFillMapWithCov, gkTrackFillMapWithCov>(event, groupedBarrelAssocs, tracks, groupedDielectrons);
    }
  }
  ...
  // add by Yuanjing
  void processJpsiProtonMEFemto(soa::Filtered<MyEventsHashSelected>& events,
                               soa::Filtered<soa::Join<aod::ReducedTracksAssoc, aod::BarrelTrackCuts>> const& assocs,
  ...
  // 20250725
  PROCESS_SWITCH(AnalysisDileptonTrack, processJpsiProtonMEFemto, "Run barrel dilepton-hadron mixed event pairing for femto", false);
  // 20250725
  PROCESS_SWITCH(AnalysisDileptonTrack, processJpsiMEProtonMEFemto, "Run barrel dilepton(ME)-hadron mixed event pairing for femto", false);
